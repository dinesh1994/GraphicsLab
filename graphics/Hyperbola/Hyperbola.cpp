...
void setupMenus(HWND hwnd)
{
    ...
    {
    case READY_MODE :
        // enable Hyperbola menu
        EnableMenuItem(hMenu, ID_HYPERBOLA,
                       MF_BYCOMMAND | MF_ENABLED);
        break;
    case DRAW_MODE :
    case DRAWN_MODE :
        // disable Hyperbola menu
        EnableMenuItem(hMenu, ID_HYPERBOLA,
...
void drawHyperbola()
{
    ...
    while (x > 1)
    {
        if (d < 0)
        {
            ...
            // p1_(k+1) = p1_k + 2*(ry^2)*x_(k+1) + ry^2
            // 2*x_(k+1) + 1 = 2*x_k + 3
            d += x - 0.5;

            // new pt. is [x_k, y_(k+1)]
            y += 1;
        }
        else
        {
            ...
            // p1_(k+1) = p1_k + 2*(ry^2)*x_(k+1) + ry^2 - 2*(rx^2)*y_(k+1)
            // 2*x_(k+1) + 1 = 2*x_k + 3
            // -2*y_(k+1) = 2 - 2*y_k
            d += x - y - 0.5;

            // new pt. is [x_(k+1), y_(k-1)]
            x -= 1;
            y += 1;
        }
        plot_sympoint(x, y, RGB(0, 0, 0));
    ...
    // p2_0 = ry^2*(x0 + 1/2)^2 + (rx^2)*(y0 - 1)^2 - (rx^2)(ry^2)
    d = 0;
    ...
    while (y > 1)
    {
        ...
        {
            /*choose SE*/
            // p2_(k+1) = p2_k - 2*(rx^2)*y_(k+1) + 2*(ry^2)*x_(k+1) + rx^2
            // 1 - 2*y_(k+1) = 3 - 2*y_k
            d += y - 0.5;
        ...
        {
            /*choose S*/
            // p2_(k+1) = p2_k - 2*(rx^2)*y_(k+1) + rx^2
            // 1 - 2*y_(k+1) = 3 - 2*y_k
            d += y - x - 0.5;
